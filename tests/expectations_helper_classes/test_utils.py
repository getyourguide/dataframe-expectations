...
def test_requires_params_missing_param():
    ...
    with pytest.raises(ValueError) as context:
        func(a=1)
    assert "missing required parameters" in str(context.value), f"Expected 'missing required parameters' in error message but got: {str(context.value)}"
...
def test_requires_params_type_error():
    ...
    assert "type validation errors" in str(context.value), f"Expected 'type validation errors' in error message but got: {str(context.value)}"
